...
module.exports = {
    ...
    ftpDownload(read, dir) {
        return new Promise((good, bad) => {

            ftpQueue.push(function (endFtpQueueItem) {

                const newPath = path.join(dir, read.fileName);
                const filePath = '/vol1/' + read.ftpPath;

                const c = new Client();
                c.on('ready', function () {
                    // console.log('want to download', myURL.pathname);
                    console.log('Downloading', filePath, 'to', newPath);
                    c.get(filePath, function (err, stream) {
                        if (err) {
                            endFtpQueueItem();
                            return bad(new Error(`Could not get file ${filePath}`));
                        } else {
                ...
                const address = 'ftp.sra.ebi.ac.uk';
                c.connect({host: address});
            })
        })
    }
